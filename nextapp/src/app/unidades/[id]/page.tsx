...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    keywords: [unidade.nome.toLowerCase(), "unidade vip esportes", ...unidade.modalidades.map((m) => `aulas ${m.toLowerCase()}`)],
    openGraph: { images: unidade.imagens?.[0] ? [{ url: unidade.imagens[0] }] : [] },
  };
}
...
export default async function UnidadePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const mapsUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(unidade.endereco || `${unidade.local} Rio de Janeiro`)}`;
        ...
        <div className="container">
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {unidade.modalidades.map((mod) => (
              <span key={mod} className="text-sm bg-turquesa/20 text-turquesa px-3 py-1 rounded-full font-medium">{mod}</span>
            ))}
            {unidade.tipos.map((tipo) => (
              <span key={tipo} className="text-sm bg-white/10 text-white/80 px-3 py-1 rounded-full font-medium">{tipo}</span>
            ))}
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {unidade.imagens.map((img, i) => (
                      <div key={i} className="aspect-[4/3] rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                        <img src={img} alt={`Foto ${i + 1} da Unidade ${unidade.nome}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" loading="lazy" />
                      </div>
                    ))}
                  </div>
                </>
            ...
            <div>
              <div className="bg-gray-50 border border-gray-100 rounded-xl p-6 sticky top-28">
                ...
                <a href={getWhatsAppUrl(unidade.whatsappKey)} target="_blank" rel="noopener noreferrer" className="w-full inline-flex items-center justify-center gap-2 bg-turquesa hover:bg-turquesa-dark text-white font-semibold px-6 py-3 rounded-lg transition-all duration-200 active:scale-[0.97]">
                  {unidade.botaoTexto}
                </a>
                ...
                <div className="mt-6 pt-6 border-t border-gray-200">
                  ...
                  <a href={mapsUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-turquesa hover:text-navy font-medium transition-colors">
