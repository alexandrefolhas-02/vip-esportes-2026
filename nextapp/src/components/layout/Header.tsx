...
export default function Header() {
  ...
  const isModalidadesActive = () => pathname.startsWith("/modalidades/");
  const isUnidadesActive = () => pathname === "/unidades" || pathname.startsWith("/unidades/");
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#000040]/95 backdrop-blur-md border-b border-white/10">
      <div className="container flex items-center justify-between h-16 lg:h-20">
        ...
        <Link href="/" className="flex items-center">
          <img src="/logo-vip-esportes.svg" alt="VIP Esportes" className="h-10 lg:h-12 w-auto" />
        </Link>
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-[#000040] border-t border-white/10 max-h-[80vh] overflow-y-auto">
          <nav className="container py-4 flex flex-col gap-1">
            <Link href="/" onClick={() => setMobileOpen(false)} className="px-3 py-3 text-white/90 hover:text-turquesa font-medium">Início</Link>
            ...
            {unidadesOpen && (
              <div className="pl-6 flex flex-col gap-1">
                <Link href="/unidades" onClick={() => setMobileOpen(false)} className="px-3 py-2 text-sm text-turquesa font-medium">Ver todas</Link>
                {unidades.map((uni) => (
            ...
            <Link href="/metodologia" onClick={() => setMobileOpen(false)} className="px-3 py-3 text-white/90 hover:text-turquesa font-medium">Metodologia</Link>
            <Link href="/para-escolas-clubes-condominios" onClick={() => setMobileOpen(false)} className="px-3 py-3 text-white/90 hover:text-turquesa font-medium">Para Escolas, Clubes e Condomínios</Link>
            <Link href="/sobre" onClick={() => setMobileOpen(false)} className="px-3 py-3 text-white/90 hover:text-turquesa font-medium">A VIP</Link>
            <Link href="/escola-de-volei-paula-pequeno" onClick={() => setMobileOpen(false)} className="px-3 py-3 text-turquesa font-semibold">Escola de Vôlei Paula Pequeno</Link>
            <Link href="/blog" onClick={() => setMobileOpen(false)} className="px-3 py-3 text-white/90 hover:text-turquesa font-medium">Blog</Link>
